...
async function forceUpdateAllConcerts() {
  try {
    console.log('Starting FORCED update for ALL concerts with latest model and coordinates...');
    console.log('Fetching all artists and their concerts...');
    ...
    for (const artist of artists) {
      for (const concert of artist.concerts) {
        ...
        // Call the ML processor - FIXED PATH
    ...
    console.log(`Updated/created predictions: ${updatedCount}`);
    console.log(`Errors: ${errorCount}`);

    if (processedCount > 0) {
      console.log(`\n✅ Forced update completed successfully!`);
      console.log(`All concerts now have the latest prediction (heuristic-demand-v4-improved) with coordinates.`);
    }
