...
async function main() {
  ...
  // Find Arijit Singh
  const arijit = artists.find(a =>
    a.artistName.toLowerCase().includes('arijit singh') ||
    (a.displayName && a.displayName.toLowerCase().includes('arijit singh'))
  );

  if (!arijit) {
    console.log('Arijit Singh not found');
    return;
  }

  console.log('');
  console.log('Arijit Singh found:', {
    id: arijit.id,
    artistName: arijit.artistName,
    displayName: arijit.displayName
  });

  // Get Arijit's platform data
  const arijitRow = PLATFORMS.map(platform => toNumber(arijit[platform]));
  const arijitTransformed = arijitRow.map(transformReachValue);

  // Normalize Arijit's data
  const arijitNormalized = arijitTransformed.map((value, index) => {
    const max = maxByPlatform[PLATFORMS[index]];
    return max > 0 ? value / max : 0;
  });
  ...
  for (let i = 0; i < PLATFORMS.length; i++) {
    entropyScore += arijitNormalized[i] * weights[PLATFORMS[i]];
  }
  ...
  PLATFORMS.forEach((platform, index) => {
    console.log(platform + ': ' + arijitRow[index] + ' (normalized: ' + arijitNormalized[index].toFixed(4) + ')');
  });
  ...
  console.log('');
  console.log('Arijit Singh Entropy Popularity Score: ' + finalScore);
