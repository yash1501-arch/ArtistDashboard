...
async function updateConcertCoordinates() {
  try {
    console.log('Starting to update missing concert coordinates...');

    // Find concerts with missing coordinates
    const concertsWithoutCoords = await prisma.concert.findMany({
      where: {
        OR: [
          { latitude: null },
          { longitude: null }
    ...
    console.log(`Found ${concertsWithoutCoords.length} concerts with missing coordinates`);

    let updatedCount = 0;
    let skippedCount = 0;

    for (const concert of concertsWithoutCoords) {
      // Try to get coordinates from city
      const cityCoords = getCoordinatesForCity(concert.city);

      let needsUpdate = false;
      const updateData: any = {};

      if (concert.latitude === null && cityCoords) {
        updateData.latitude = cityCoords.lat;
        needsUpdate = true;
      }

      if (concert.longitude === null && cityCoords) {
        updateData.longitude = cityCoords.lng;
        needsUpdate = true;
      }
      ...
      if (needsUpdate) {
        ...
        updatedCount++;
        console.log(`Updated ${concert.artistName || 'Unknown'} - ${concert.venueName || 'Unknown Venue'} (${concert.city})`);
      } else {
        skippedCount++;
        // Only log every 10th skipped to avoid spam
        if (skippedCount % 10 === 0) {
          console.log(`Skipped ${skippedCount} concerts (no coordinates available for city: ${concert.city})`);
        }
      }
    }
    ...
    console.log(`  Skipped: ${skippedCount} concerts (no coordinates found)`);

  } catch (error) {
    console.error('Error updating concert coordinates:', error);
  } finally {
